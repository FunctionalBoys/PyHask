...
getNextAddress StringType _ = Nothing

getNextVariableAddress :: SimpleType -> ParserState -> Maybe Address
getNextVariableAddress sType (ParserState ((Scope _ _ _ memoryblock _) N.:| _) _ _ _) = getNextAddress sType memoryblock
...
increaseCurrentAddress StringType (MemoryBlock tMBI tMF tMC tMB) = MemoryBlock tMBI tMF tMC tMB